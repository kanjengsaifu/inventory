...
select NULL,a.id_glasir, 'AH00002', 'admin', 'Zainudin', 2, '2015-12-02', '13:00:00', 0, '2015-10-30', '2015-11-20', '18:00:00', ' 18:00:00', '2015-11-20 13:00:00',	'0000-00-00 00:00:00',	'0000-00-00 00:00:00',
                        ...
                        (
                          select id_glasir, 
                          REPLACE(FORMAT(COALESCE(sum(case when deleted <> 1 AND tgl_combine between '2015-10-30 18:00:00' AND '2015-11-20 18:00:00' THEN (1.565*((densitas-1000)/1000)*volume) ELSE 0 END), 0),2),',','') turun_bgps
                          from glasir_phd
                          where deleted = 0
                          group by id_glasir
                        ...
                        (
                          select id_glasir, 
                          REPLACE(FORMAT(COALESCE(sum(case when deleted <> 1 AND tgl_combine between '2015-10-30 18:00:00' AND '2015-11-20 18:00:00' THEN (1.565*((densitas-1000)/1000)*volume) ELSE 0 END), 0),2),',','') ditarik_supply
                          from glasir_phd_sp
                          where deleted = 0
                          group by id_glasir
                        ...
                        (
                          select id_glasir, 
                          REPLACE(FORMAT(COALESCE(sum(case when deleted <> 1 AND tgl_combine between '2015-10-30 18:00:00' AND '2015-11-20 18:00:00' THEN (1.565*((densitas-1000)/1000)*volume) ELSE 0 END), 0),2),',','') return_prod
                          from glasir_rhd
                          where deleted = 0
                          group by id_glasir
                        ...
                        (
                          select id_glasir, 
                          REPLACE(FORMAT(COALESCE(sum(case when deleted <> 1 AND tgl_combine between '2015-10-30 18:00:00' AND '2015-11-20 18:00:00' THEN (1.565*((densitas-1000)/1000)*volume) ELSE 0 END), 0),2),',','') kirim_prod
                          from glasir_thd
                          where deleted = 0
                          group by id_glasir
