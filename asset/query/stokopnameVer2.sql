SELECT a.id_glasir,a.nama_glasir,
	REPLACE(FORMAT(COALESCE(sum(case when c.deleted <> 1 THEN g.c_ds_bgps ELSE 0 END), 0),2),',','') as adj_bgps,
	REPLACE(FORMAT(COALESCE(sum(case when c.deleted <> 1 THEN g.c_ds_sply ELSE 0 END), 0),2),',','') as adj_sply,
	REPLACE(FORMAT(COALESCE(sum(CASE WHEN b.area=2 AND b.deleted <> 1 AND b.inspected = 1 AND b.period = 
		1 THEN b.bkg ELSE 0 END), 0),2),',','') as sab, 
	REPLACE(FORMAT(COALESCE(sum(CASE WHEN b.area=3 AND b.deleted <> 1 AND b.inspected = 1 AND b.period = 
		1 THEN b.bkg ELSE 0 END), 0),2),',','') as sas, 
