		...
		return $result;
         ...
         public function get_stok(){
                $query ="SELECT a.id_glasir,a.nama_glasir, 
                        FORMAT(COALESCE(sum(CASE WHEN b.area=2 AND b.deleted <> 1 AND b.inspected = 1 AND b.period = 
                                (SELECT DISTINCT MAX(g.period)  FROM glasir_ohd g) THEN b.bkg ELSE 0 END), 0),2) as sab, 
                        FORMAT(COALESCE(sum(CASE WHEN b.area=3 AND b.deleted <> 1 AND b.inspected = 1 AND b.period = 
                        ...
                        FORMAT(COALESCE(sum(case when c.deleted <> 1 THEN (1.565*((c.densitas-1000)/1000)*c.volume) ELSE 0 END), 0),2) as turun_bgps,
                        FORMAT(COALESCE(sum(case when d.deleted <> 1 THEN (1.565*((d.densitas-1000)/1000)*d.volume) ELSE 0 END), 0),2) as ditarik_supply,
                        FORMAT(COALESCE(sum(case when e.deleted <> 1 THEN (1.565*((e.densitas-1000)/1000)*e.volume) ELSE 0 END), 0),2) as return_prod,
                        FORMAT(COALESCE(sum(case when f.deleted <> 1 THEN (1.565*((f.densitas-1000)/1000)*f.volume) ELSE 0 END), 0),2) as kirim_prod,

                        FORMAT(((COALESCE(sum(case when c.deleted <> 1 THEN (1.565*((c.densitas-1000)/1000)*c.volume) ELSE 0 END), 0)-COALESCE(sum(case when d.deleted <> 1 THEN (1.565*((d.densitas-1000)/1000)*d.volume) ELSE 0 END), 0))+COALESCE(sum(CASE WHEN b.area=2 AND b.deleted <> 1 AND b.inspected = 1 AND b.period = (SELECT DISTINCT MAX(g.period)  FROM glasir_ohd g) THEN b.bkg ELSE 0 END), 0)),2) as stok_bgps,
