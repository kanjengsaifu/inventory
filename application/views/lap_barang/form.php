...
$(document).ready(function(){
	...
	$("#cari").click(function(){
		...
		var jenis = $("#cbo_jenis").val();
		var	pilih	= $(".pilih:checked").val();
		var jml_pilih = $(".pilih:checked");
		
		
		var string = "kode="+kode+"&pilih="+pilih+"&jenis="+jenis;
	...
	$("#cetak").click(function(){
		...
		window.open('<?php echo site_url();?>/lap_barang/cetak/'+pilih+'/'+kode+'/'+jenis);
		return false();
	});
});	
</script>
...
<tr>    
	<td width="150"><input type="radio" name="pilih" class="pilih" value="jenis" />Jenis  Barang</td>
    <td width="5"></td>
    <td>
    <select name="cbo_jenis" id="cbo_jenis">
    <option value="">-Pilih-</option>
    <?php
	$dt_jenis = $this->app_model->jenis_barang();
	foreach($dt_jenis->result() as $dt){
	?>
    	<option value="<?php echo $dt->id_jenis;?>"><?php echo $dt->jenis;?></option>
    <?php
	}
	?>
    </select>
