...
function myfooter(){	
	...
	foreach($data->result_array() as $r){
		...
		*/
		$stok_g=0;
		$jenis = $this->app_model->nama_jenis_barang($r['id_jenis']);
		$row = $this->app_model->row_detail($r['id_jenis']);
		/*
		$dt_group = $this->app_model->group_row_detail($r['id_jenis']);
		$no_g = 0;
		foreach($dt_group->result() as $dt){
			$no_g = $no_g+1;
		}
		*/
	if(($no%25) == 1){
	...
	?>
    <tr>
    	<td colspan="11" align="center"><b>Jenis <?php echo $jenis;?> </b></td>
    </tr>     
    <?php 
	$data_detail =   $this->app_model->group_row_detail($r['id_jenis']);
	foreach($data_detail->result_array() as $r){
		$awal = $this->app_model->CariStokAwal($r['kode_barang']);
		$beli = $this->app_model->CariJmlBeli($r['kode_barang']);
		$jual = $this->app_model->CariJmlJual($r['kode_barang']);	
		$stok = $this->app_model->CariStokAkhir($r['kode_barang']);
    ...
    <?php
		...
		<tr>
			...
			<td align="center"><b><?php echo $stok_g;?></b></td>
		</tr>
		<?php
