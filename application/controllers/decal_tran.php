...
class Decal_tran extends CI_Controller {
	 ...
	 * @keterangan  : Controller halaman decal transit
	 **/
        ...
        public function simpan()
	{
		...
		if(!empty($cek)){
				...
				if($data->num_rows()>0){
						...
						$data = $this->dclModel->getSelectedData("decal_thd",$id_d);
						if($data->num_rows()>0){
							...
							$this->dclModel->insertData("decal_thd",$ud);
	...
	public function edit()
	{
		...
		if(!empty($cek)){
			...
			$d['judul'] = "Ubah input stock transit decal";
        ...
        public function editDetail()
	{
		...
		if(!empty($cek)){
			...
			$text = "SELECT a.id as batch,a.shift,a.size_kertas,a.size_kat,a.id_related,a.no_po,a.id_decal_items,e.nama,TIME_FORMAT(a.jam,'%H:%i') as jam,i.nama as jenis,
                                    a.tgli,a.warna,a.id_bm,a.id_bmt,a.komposisi,d.jns_bm as nama_bmt,e.nama as decal_item,c.nama_bm,a.kw1,a.kw2,a.kw3,a.petugas,a.inputer, a.jenis_decal,a.penerima
                                    FROM decal_thd as a
                                    LEFT JOIN decal_th as b ON a.id_related=b.id 
        ...
        public function DataDetail()
	{
		...
		if(!empty($cek)){
			...
			$text = "SELECT a.id as batch,a.id_related,a.no_po,a.id_decal_items,e.nama,f.nama as shift ,TIME_FORMAT(a.jam,'%H:%i') as jam,i.nama as jenis, g.dsc as size_kertas, h.dsc as size_kat,
                                    a.tgli,a.warna,a.komposisi,e.nama as decal_item,a.kw1,a.kw2,a.kw3,a.petugas,a.penerima,a.inputer
                                    FROM decal_thd as a 
                                    LEFT JOIN decal_th as b ON a.id_related=b.id 
                                    LEFT JOIN decal_items as e ON e.id=a.id_decal_items
                                    LEFT JOIN global_shift as f ON a.shift=f.id
	...
	public function hapus_detail()
	{
		...
		if(!empty($cek)){                        
                                ...
                                $batch = $this->uri->segment(5);
                                $tgl_deleted = date('Y-m-d h:i:s');
                                $this->dclModel->manualQuery("UPDATE decal_thd SET deleted = 1,tgl_delete = '$tgl_deleted' WHERE id_related='$id' AND id_decal_items='$kode' AND id='$batch'");
                                $this->edit();
