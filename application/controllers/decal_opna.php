...
class Decal_opna extends CI_Controller {
	...
	public function index()
	{
		...
		if(!empty($cek)){
			...
			$d['judul']="Daftar input opname decal";
	...
	public function tambah()
	{
		...
		if(!empty($cek)){
			...
			$d['judul']="Input opname decal";
			
			$id         = $this->dclModel->MaxPhDecalOpna();
                        $id_group   = $this->dclModel->MaxOhGroup();
        ...
        public function simpan()
	{
		...
		if(!empty($cek)){
				...
				if($data->num_rows()>0){
						...
						$datax = $this->dclModel->getSelectedData("decal_ohd",$id_d);
						if($datax->num_rows()>0){
                                                        //$ud['tgl_update']   = date('Y-m-d h:i:s');
							//$this->dclModel->updateData("decal_ohd",$ud,$id_d);

                                                        $sql = "update decal_ohd set jml = '$jmlx'*isi_motif, tgli = '$tglix', petugas = '$petugasx' where id_related = '$idx' and id_group = '$id_groupx' and parent_id = '$parent_idx'";
                                                        $this->db->query($sql);
        ...
        public function simpanItem()
	{
		...
		if(!empty($cek)){
                                ...
                                $sql = "update decal_ohd set rusak = '$rusak', jml = '$jml', tgl_update = '$tgl_update' where id_related = '$id' and id_group = '$id_group' and parent_id = '$parent_id'
                                         and id = '$batch' and item_code = '$item_code'";
                                $this->db->query($sql);
                                echo "<meta http-equiv='refresh' content='0; url=".base_url()."index.php/decal_opna/edit/$id'>";
                                echo 'Update data Sukses';
		}else{
				header('location:'.base_url());
		}
        ...
        public function editItem()
	{
		...
		if(!empty($cek)){
			...
			$d['usaha']= $this->config->item('usaha');
			$d['alamat_instansi']= $this->config->item('alamat_instansi');
                        $d['judul'] = "Ubah item opname decal";
			...
			$d['content'] = $this->load->view('decal_opna/formItem', $d, true);		
			$this->load->view('home',$d);
		}else{
			header('location:'.base_url());
		}
	...
	public function hapus_detail()
	{
		...
		if(!empty($cek)){                        
                                ...
                                $id_group = $this->uri->segment(5);
                                $tgl_deleted = date('Y-m-d h:i:s');
                                $this->dclModel->manualQuery("UPDATE decal_ohd SET deleted = 1,tgl_delete = '$tgl_deleted' WHERE id_related='$id_related' AND parent_id='$parent_id' AND id_group='$id_group'");
                                $this->edit();
        ...
        public function getPicOpnaDecal()
	{
		...
		$data['data_passed'] = $this->dclModel->getPicOpnaDecal();
