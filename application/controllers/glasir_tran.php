...
class Glasir_tran extends CI_Controller {
	...
	public function index()
	{
		...
		if(!empty($cek)){
			...
			$text = "SELECT b.nama_glasir, a.tgl_insert,a.no_prod,a.inputer,a.id_glasir,a.tgl,a.tglp,a.tglb FROM glasir_thd a 
                                ...
                                JOIN glasir_th c on a.no_prod = c.no_prod
                                $where ";		
	...
	public function hapus()
	{
		...
		if(!empty($cek)){
                                $id = $this->uri->segment(3);
                                $this->glzModel->manualQuery("UPDATE glasir_thd SET deleted = 1 WHERE no_prod='$id'");
                                $this->glzModel->manualQuery("UPDATE glasir_th SET deleted = 1  WHERE no_prod='$id'");
	...
	public function hapus_detail()
	{
		...
		if(!empty($cek)){
				$id = $this->uri->segment(3);
                                ...
                                $batch = $this->uri->segment(5);
                                $this->glzModel->manualQuery("UPDATE glasir_thd SET deleted = 1 WHERE no_prod='$id' AND id_glasir='$kode' AND idthd='$batch'");
                                $this->edit();
		}else{
			header('location:'.base_url());
		}
