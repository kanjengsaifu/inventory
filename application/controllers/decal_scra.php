...
class Decal_scra extends CI_Controller {
	...
	public function index()
	{
		...
		if(!empty($cek)){
			...
			$d['judul']="Daftar input scrap decal";
			...
			$config['base_url'] = site_url() . '/decal_scra/index/';
			...
			$d['content'] = $this->load->view('decal_scra/view', $d, true);		
	...
	public function tambah()
	{
		...
		if(!empty($cek)){
			...
			$d['judul']="Input scrap decal";
			
			$id         = $this->dclModel->MaxPhDecalScra();
                        $id_group   = $this->dclModel->MaxShGroup();
			...
			$d['content'] = $this->load->view('decal_scra/form', $d, true);		
			$this->load->view('home',$d);
		}else{
			header('location:'.base_url());
		}
        ...
        public function simpan()
	{
		...
		if(!empty($cek)){
				...
				$data = $this->dclModel->getSelectedData("decal_sh",$id);
				if($data->num_rows()>0){
                                                $up['tgl_update']		= date('Y-m-d h:i:s');
                                                $this->dclModel->updateData("decal_sh",$up,$id);
						$datax = $this->dclModel->getSelectedData("decal_shd",$id_d);
						if($datax->num_rows()>0){
                                                        //$ud['tgl_update']   = date('Y-m-d h:i:s');
							//$this->dclModel->updateData("decal_shd",$ud,$id_d);

                                                        $sql = "update decal_shd set jml = '$jmlx'*isi_motif, tgli = '$tglix', petugas = '$petugasx', area = '$areax' where id_related = '$idx' and id_group = '$id_groupx' and parent_id = '$parent_idx'";
                                                        $this->db->query($sql);
                                                        echo "<meta http-equiv='refresh' content='0; url=".base_url()."index.php/decal_scra/edit/$idx'>";
                                                        echo 'Update data Sukses';
						}else{
                                                        $tgl_inputx		        = date('Y-m-d h:i:s');
                                                        $sql = "insert into decal_shd (id,id_group,id_related,parent_id, item_code, isi_motif,jml,rusak,shift,id_bm,id_bmt,area,tgli,jam,petugas,inputer,
                                        ...
                                        $tgl_inputx		        = date('Y-m-d h:i:s');
					$this->dclModel->insertData("decal_sh",$up);
                                        $sql = "insert into decal_shd (id,id_group,id_related,parent_id, item_code, isi_motif,jml,rusak,shift,id_bm,id_bmt,area,tgli,jam,petugas,inputer,
                                                tgl_input,tgl_update,tgl_delete,deleted)
                                                select NULL,'$id_groupx','$idx',parent_id,item_code,isi_motif,isi_motif*$jmlx,0,'$shiftx','$id_bmx','$id_bmtx','$areax','$tglix','$jamx','$petugasx','$inputerx',
                                                '$tgl_inputx','0000-00-00 00:00:00','0000-00-00 00:00:00',0 from decal_items_detail where parent_id = '$parent_idx'";
        ...
        public function simpanItem()
	{
		...
		if(!empty($cek)){
                                ...
                                $sql = "update decal_shd set rusak = '$rusak', jml = '$jml', tgl_update = '$tgl_update' where id_related = '$id' and id_group = '$id_group' and parent_id = '$parent_id'
                                         and id = '$batch' and item_code = '$item_code'";
                                $this->db->query($sql);
                                echo "<meta http-equiv='refresh' content='0; url=".base_url()."index.php/decal_scra/edit/$id'>";
                                echo 'Update data Sukses';
		}else{
				header('location:'.base_url());
		}
	...
	public function edit()
	{
		...
		if(!empty($cek)){
			...
			$d['judul'] = "Ubah input scrap decal";
			
			$id = $this->uri->segment(3);
			$text = "SELECT * FROM decal_sh WHERE id='$id'";
			$data = $this->dclModel->manualQuery($text);
                        $id_group   = $this->dclModel->MaxShGroup();
			if($data->num_rows() > 0){
				foreach($data->result() as $db){
        ...
        public function editDetail()
	{
		...
		if(!empty($cek)){
			...
			$text = "select a.id,a.id_group,a.id_related,a.parent_id,a.item_code, a.isi_motif, sum(a.jml)/sum(a.isi_motif) as jml, a.rusak, a.shift,a.area, 
                                    a.id_bm, a.id_bmt, a.tgli, a.jam, a.petugas, a.inputer, a.tgl_input, a.tgl_update, a.tgl_delete, a.deleted from decal_shd a
        ...
        public function editItem()
	{
		...
		if(!empty($cek)){
			...
			$d['content'] = $this->load->view('decal_scra/formItem', $d, true);		
			$this->load->view('home',$d);
		}else{
			header('location:'.base_url());
		}
        ...
        public function DataDetail()
	{
		...
		if(!empty($cek)){
			...
			$text1 = "SELECT a.id,a.id_group,a.tgli,a.jam,a.id_related,a.parent_id,a.item_code,sum(a.isi_motif) as isi_motif,sum(a.jml) as jml,sum(a.rusak) as rusak,a.shift,a.id_bm,a.id_bmt,a.petugas,a.inputer from decal_shd a  
                                    WHERE a.id_related='$id' AND a.deleted = 0 group by a.id_group order by a.id desc";
			$d['data']= $this->dclModel->manualQuery($text1);
                        
                        $text2 = "SELECT a.id,a.id_group,a.id_related,a.parent_id,a.item_code,b.item,a.isi_motif,a.jml,a.rusak,a.shift,a.id_bm,a.id_bmt,a.petugas,a.inputer from decal_shd a 
			...
			$this->load->view('decal_scra/detail',$d);
		}else{
			header('location:'.base_url());
		}
	}
	...
	public function hapus()
	{
		...
		if(!empty($cek)){
                                ...
                                $this->dclModel->manualQuery("UPDATE decal_shd SET deleted = 1,tgl_delete = '$tgl_deleted' WHERE id_related='$id'");
                                $this->dclModel->manualQuery("UPDATE decal_sh SET deleted = 1,tgl_delete = '$tgl_deleted' WHERE id='$id'");
                                echo "<meta http-equiv='refresh' content='0; url=".base_url()."index.php/decal_scra'>";			
		}else{
			header('location:'.base_url());
		}
	}
	...
	public function hapus_detail()
	{
		...
		if(!empty($cek)){                        
                                ...
                                $id_group = $this->uri->segment(5);
                                $tgl_deleted = date('Y-m-d h:i:s');
                                $this->dclModel->manualQuery("UPDATE decal_shd SET deleted = 1,tgl_delete = '$tgl_deleted' WHERE id_related='$id_related' AND parent_id='$parent_id' AND id_group='$id_group'");
                                $this->edit();
        ...
        public function getPicScraDecal()
	{
		...
		$data['data_passed'] = $this->dclModel->getPicScraDecal();
