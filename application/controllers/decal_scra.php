				...
				if($data->num_rows()>0){
						...
						if($datax->num_rows()>0){
							...
							//$this->dclModel->updateData("decal_shd",$ud,$id_d);
                                                        ...
                                                        $sql = "update decal_shd set jml = '$jmlx'*isi_motif, tgli = '$tglix', petugas = '$petugasx' where id_related = '$idx' and id_group = '$id_groupx' and parent_id = '$parent_idx'";
                                                        $this->db->query($sql);
                                                        echo "<meta http-equiv='refresh' content='0; url=".base_url()."index.php/decal_scra/edit/$idx'>";
                                                        echo 'Update data Sukses';
						}else{
                                                        $tgl_inputx		        = date('Y-m-d h:i:s');
                                                        $sql = "insert into decal_shd (id,id_group,id_related,parent_id, item_code, isi_motif,jml,rusak,shift,id_bm,id_bmt,tgli,jam,petugas,inputer,
